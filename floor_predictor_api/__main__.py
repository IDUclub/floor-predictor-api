...
from floor_predictor_api.core.config import AppConfig, Config, LoggingConfig

LogLevel = tp.Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]


def _run_uvicorn(configuration: dict[str, tp.Any]) -> tp.NoReturn:
    """Run application with uvicorn."""
    uvicorn.run(
        "floor_predictor_api:app",
        **configuration,
    )


@click.command("Run floor-predictor api service")
@click.option(
    "--port",
    "-p",
    envvar="PORT",
...
def main(
    ...
    """
    Floor-predictor api backend service main function, performs configuration
